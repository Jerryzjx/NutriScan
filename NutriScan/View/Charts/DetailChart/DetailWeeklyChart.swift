...
struct NutrientIntakePerDay: Identifiable {
    let id = UUID()
    let day: Date
    let intake: Double
}

struct DetailWeeklyChart: View {
    ...
    private func calculateNutrientIntakePerDay(nutritionType: String) -> [NutrientIntakePerDay] {
        ...
        items.forEach { item in
            ...
            if let _ = last7DaysIntake[itemDate] {
                ...
                last7DaysIntake[itemDate, default: 0.0] += intakeValue
            }
        }

        // Convert to [NutrientIntakePerDay]
        return last7DaysIntake.map { NutrientIntakePerDay(day: $0.key, intake: $0.value) }.sorted(by: { $0.day < $1.day })
    }
  ...
  var dailyValueDescription: String {
    ...
    var body: some View {
        let nutriThisWeek = vm.averageIntakePastSevenDays(nutrientType: nutrientType, items: items)
        ScrollView {
            VStack (alignment: .leading, spacing: 4){
                ...
                VStack (alignment: .leading, spacing: 2){
                    ...
                    HStack (alignment: .firstTextBaseline ,spacing: 4){
                        Text("\(nutriThisWeek, specifier: "%.0f")")
